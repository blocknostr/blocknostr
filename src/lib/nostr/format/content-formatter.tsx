...
import { safeExecute } from '@/lib/utils/errorHandling';
    ...
    try {
      ...
      const formattedParts = parts.map((part, index) => {
        try {
          ...
          console.warn("Error processing content part:", part, error);
          return part; // Fallback to plain text on error
        }
      });
      ...
      // Fallback to plain text on complete format failure
      return <>{content}</>;
    }
  },
  ...
  extractMentionedPubkeys: (content: string, tags: string[][]) => {
    try {
      ...
      if (tags && Array.isArray(tags)) {
        tags.forEach(tag => {
          if (Array.isArray(tag) && tag.length >= 2 && tag[0] === 'p' && tag[1]) {
            pubkeys.push(tag[1]);
          }
        });
      }
        ...
        if (matches) {
          matches.forEach(match => {
            try {
              ...
              console.warn("Error extracting pubkey from URL:", match, error);
            }
          });
        }
      }
