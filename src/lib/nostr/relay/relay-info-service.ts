...
export class RelayInfoService {
   ...
   */
  public async getRelayLimitations(relayUrl: string): Promise<Record<string, unknown> | null> {
    const info = await this.getRelayInfo(relayUrl);
   ...
   */
  private isValidRelayInfo(info: RelayInfo): boolean {
...
export interface RelayInfo {
  ...
  supportedNIPs: number[]; // Made mandatory to match relay-manager.ts
