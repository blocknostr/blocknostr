...
export const extractMediaFromTags = (tags: string[][]): MediaItem[] => {
  ...
  return mediaTags.map(tag => {
    ...
    if (tag[0] === 'video' || tag[1].match(/\.(mp4|webm|mov|m4v|ogv|avi|wmv|mkv|flv)(\?.*)?$/i)) {
      mediaItem.type = 'video';
    } else if (tag[0] === 'audio' || tag[1].match(/\.(mp3|wav|ogg|flac|aac|m4a)(\?.*)?$/i)) {
      mediaItem.type = 'audio';
    } else if (tag[0] === 'image' || tag[0] === 'imeta' || tag[1].match(/\.(jpg|jpeg|png|gif|webp|avif|svg|bmp|tiff)(\?.*)?$/i)) {
