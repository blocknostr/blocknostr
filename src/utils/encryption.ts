...
export const encryption = {
  ...
  async decryptContent(encryptedContent: string, senderPubkey: string): Promise<string | null> {
    try {
      // Check if NIP-04 is available using the utility function
      if (this.isNip04Available()) {
        const decrypted = await window.nostr.nip04.decrypt(senderPubkey, encryptedContent);
        return decrypted;
      } else {
        // Silently return null if NIP-04 is not available
        // This is expected when no Nostr extension is installed
        return null;
      }
    } catch (error) {
      // Only log the error in development mode to avoid console spam
      if (process.env.NODE_ENV === 'development') {
        console.warn("NIP-04 decryption failed:", error);
        console.info("NIP-04 status:", this.getNip04StatusMessage());
      }
      return null;
    }
  },
  ...
  /**
   * Get a user-friendly message about NIP-04 availability
   */
  getNip04StatusMessage(): string {
    if (typeof window === 'undefined') {
      return "Running in server environment - NIP-04 not available";
    }
    
    if (!window.nostr) {
      return "No Nostr extension detected. Please install a Nostr browser extension like Alby, nos2x, or Flamingo to decrypt encrypted notes.";
    }
    
    if (!window.nostr.nip04) {
      return "Your Nostr extension doesn't support NIP-04 encryption. Please update your extension or use one that supports NIP-04.";
    }
    
    return "NIP-04 decryption is available";
  },
};
