  ...
  useEffect(() => {
    const trackScrollVelocity = () => {
      ...
      if (timeDiff > 0) {
        ...
        if (scrollVelocity.current > 1.0 && // Increased threshold from 0.5 to 1.0 (less aggressive)
    ...
    async (entries: IntersectionObserverEntry[]) => {
      ...
      if (target?.isIntersecting && hasMore && !disabled && !isFetchingRef.current) {
        ...
        try {
          ...
          // TEMPORARILY DISABLED: Scroll position adjustment to prevent forced scrolling
          // if (preservePosition) {
