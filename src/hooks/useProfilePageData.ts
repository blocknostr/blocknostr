...
export const useProfilePageData = (npub?: string) => {
  const [profileData, setProfileData] = useState<any | null>(null);
  const [events, setEvents] = useState<NostrEvent[]>([]);
  const [replies, setReplies] = useState<NostrEvent[]>([]);
  const [media, setMedia] = useState<NostrEvent[]>([]);
  const [reposts, setReposts] = useState<{ originalEvent: NostrEvent; repostEvent: NostrEvent }[]>([]);
  ...
  }, [npub, isCurrentUser, originalPostProfiles, toast]);
