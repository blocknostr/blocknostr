...
export function useNoteEditorState(onNoteSaved: (note: Note) => void) {
  ...
  useEffect(() => {
    try {
      ...
      const existingKey = localStorage.getItem('notes_encryption_key');
      if (existingKey) {
      ...
      try {
        localStorage.setItem('notes_encryption_key', key);
      } catch (storageError) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const uniqueId = noteId || `note-${Math.random().toString(36).substring(2, 10)}`;
  ...
  const shareNote = () => {
    ...
    const shareUrl = `${window.location.origin}/notes?note=${noteId}`;
