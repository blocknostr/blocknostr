...
export function useNotes() {
  ...
  const [savedNotes, setSavedNotes] = useLocalStorage<Note[]>('notes_saved_notes', []);
  ...
  const allNotes = [...savedNotes, ...noteFetcher.notesCollection];
  ...
  useEffect(() => {
    ...
    console.log("Current fetched notes:", noteFetcher.notesCollection);
  }, [savedNotes, noteFetcher.notesCollection]);

  // Load view preference from localStorage
  useEffect(() => {
    const savedView = localStorage.getItem("notes_view");
